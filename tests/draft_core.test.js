...
test("第3選定の儀の完了時にヤマタノオロチへディエスイレを1枚だけ付与する", () => {
  const state = createDraftState({ round: 3, cardNos: pool(20), seed: 777, orochiSeats: [2, 2] });
  for (let step = 0; step < 5; step++) {
    for (let seat = 1; seat <= 4; seat++) {
      applyDraftPick(state, seat, privateDraftState(state, seat).pack.cards[0]);
    }
  }
  assert.equal(state.decks[2].filter(no => String(no) === "198").length, 1);
  assert.equal(state.decks[2].length, 6);
  assert.equal(state.decks[1].includes("198"), false);
  assert.deepEqual(publicDraftState(state).diesIraeGrantedSeats, [2]);
  assert.equal(privateDraftState(state, 2).deck.includes("198"), true);
});
...
test("深化開始は勝敗に応じた見識を一度だけ付与する", () => {
  ...
  const session = { seed: 123, seats: { 1: {}, 2: {}, 3: {}, 4: {} }, battles: { 1: { t1Winner: 1, t2Winner: 3 } } };
